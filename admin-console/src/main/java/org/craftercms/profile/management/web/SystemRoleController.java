...
//        mav.addObject("tenant",tenant);
    ...
    public String mapRole(
        ...
        if (!bindingResult.hasErrors()) {
        	try {
        		...
        		bindingResult.rejectValue("roleName", "role.name.validation.already.exist", null, "role.name.validation.already.exist");
        		RequestContext context = RequestContext.getCurrent();
                model.addAttribute("currentuser", context.getAuthenticationToken().getProfile());
    ...
    public ModelAndView getItem(@RequestParam(required=false) String id, @RequestParam(required=false) String roleName) throws Exception {
		ModelAndView mav = new ModelAndView();
    ...
    @ModelAttribute
    public ModelAndView deleteAccount(@ModelAttribute("role") Role role,
    		BindingResult bindingResult,
