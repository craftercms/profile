...
public class GroupRoleMappingController {
    ...
    public String mapGroupRole(@ModelAttribute("tenant") Tenant tenant, @ModelAttribute("group") GroupRole groupRole,
        ...
        if (!bindingResult.hasErrors()) {
            try {
                ...
                bindingResult.rejectValue("name", "grouprole.mapping.roles.fields.errors.groupname.already.exist",
                    null, "grouprole.mapping.roles.fields.errors.groupname.already.exist");
                List<String> roleOption = tenant.getRoles();
                RequestContext context = RequestContext.getCurrent();
    ...
    public ModelAndView getItem(@RequestParam(required = false) String id, @RequestParam(required = false) String
        tenantName, @ModelAttribute("tenant") Tenant tenant) throws Exception {
        ModelAndView mav = new ModelAndView();
        tenant = tenantDAOService.getTenantByName(tenant.getTenantName());
    ...
    public String updateGroups(@ModelAttribute("tenant") Tenant tenant, @ModelAttribute("group") GroupRole groupRole,
                               BindingResult bindingResult, Model model) throws Exception {
        tenant = tenantDAOService.getTenantByName(tenant.getTenantName());
        groupRole.setTenantName(tenant.getTenantName());
    ...
    public ModelAndView deleteAccount(@RequestParam("item") ArrayList<String> item,
                                      @ModelAttribute("tenant") Tenant tenant, HttpServletResponse response) throws
        Exception {
